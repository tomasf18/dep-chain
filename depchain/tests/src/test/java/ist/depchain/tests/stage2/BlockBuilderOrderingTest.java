...
/**
 * Unit tests for {@link BlockBuilder#orderTransactions(List)}.
 *
...
class BlockBuilderOrderingTest {
    ...
    void fourSendersInterleavedByFeeAndNonce() {
        ...
        // Round 1 eligible: Alice(0)=100k, Bob(0)=200k, Carol(0)=150k, Dave(0)=80k → Bob(0)
        assertSender(BOB, ordered.get(0));
        assertEquals(0, ordered.get(0).getNonce());

        // Round 2 eligible: Alice(0)=100k, Bob(1)=10k, Carol(0)=150k, Dave(0)=80k → Carol(0)
