...
/**
 * Contract execution consistency across replicas (TODO-TESTS §F).
 *
 ...
 *
 * Tests:
 *   1. Successful ERC-20 transfer → identical receipts, state hashes, block hashes.
...
class Erc20RevertReceiptConsistencyTest {
    ...
    void mixedSuccessAndRevertProduceIdenticalReceiptsAcrossReplicas() {
        ...
        client2Library.submitTokenTransfer(client1Address.toHexString(), BigInteger.valueOf(1000), GAS_PRICE, GAS_LIMIT);
    ...
    void revertingDecreaseAllowanceProducesConsistentStateAndReceipts() {
        ...
        client1Library.submitDecreaseAllowance(spender.toHexString(), allowanceAmount.add(BigInteger.ONE), GAS_PRICE, GAS_LIMIT);
    ...
    private static void assertLatestBlocksEquivalentAcrossReplicas() {
        ...
        for (String replicaId : REPLICAS) {
            ...
            for (int i = 0; i < referenceReceipts.size(); i++) {
                ...
                assertEquals(expected.getError(), actual.getError(), "Receipt error mismatch at index " + i + " on " + replicaId);
                assertArrayEquals(expected.getTxHash(), actual.getTxHash(), "Receipt txHash mismatch at index " + i + " on " + replicaId);
                assertArrayEquals(expected.getReturnData(), actual.getReturnData(), "Receipt returnData mismatch at index " + i + " on " + replicaId);
                assertEquals(expected.getContractAddress(), actual.getContractAddress(), "Receipt contractAddress mismatch at index " + i + " on " + replicaId);
            }
        }
    }
