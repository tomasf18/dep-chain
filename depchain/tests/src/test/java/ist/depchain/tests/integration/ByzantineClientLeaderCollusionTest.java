...
class ByzantineClientLeaderCollusionTest {
	...
	void honestReplicaRejectsTransactionFromUnknownAccount() throws Exception {
		// Byzantine client: generates a fresh key pair not registered in the world
		// state
		...
		Block malformedBlock = Block.newBuilder()
				...
				.addRequestMeta(ClientRequestMeta.newBuilder().setClientId("byzantine").setRequestId(1)
						.build())
				.build();
	...
	void honestReplicaRejectsLeaderTamperedTransactionContent() throws Exception {
		...
		// Byzantine leader tampers: changes the value to 999_999 but keeps the original
		// signature
