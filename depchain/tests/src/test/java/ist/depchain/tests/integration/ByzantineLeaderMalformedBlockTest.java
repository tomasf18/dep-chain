...
class ByzantineLeaderMalformedBlockTest {
	...
	void honestReplicaRejectsForgedTransactionInPrepare() throws Exception {
		...
		Block malformedBlock = Block.newBuilder()
				...
				.addRequestMeta(ClientRequestMeta.newBuilder().setClientId("client1").setRequestId(1)
						.build())
				.build();
		...
		assertNull(coordinator.getTree().getBlock(malformedBlock.getId()),
				"invalid prepared block must not be cached");
	...
	void honestReplicaRejectsUnsignedTransactionInjectedByLeader() throws Exception {
		...
		Block malformedBlock = Block.newBuilder()
				...
				.addRequestMeta(ClientRequestMeta.newBuilder().setClientId("client1").setRequestId(2)
						.build())
				.build();
		...
		assertNull(coordinator.getTree().getBlock(malformedBlock.getId()),
				"unsigned prepared block must not be cached");
	...
	void honestReplicaRejectsDuplicateNonceTransactionsInSameBlock() throws Exception {
		...
		Transaction tx1 = TransactionSigner.sign(
				new Transaction(owner, receiverA, BigInteger.valueOf(100), new byte[0], BigInteger.ONE,
						BigInteger.valueOf(21_000), nonce, null),
				clientContext.getPrivateKey(),
				clientConfig.getSignatureAlgorithm());
		Transaction tx2 = TransactionSigner.sign(
				new Transaction(owner, receiverB, BigInteger.valueOf(200), new byte[0], BigInteger.ONE,
						BigInteger.valueOf(21_000), nonce, null),
				clientContext.getPrivateKey(),
				clientConfig.getSignatureAlgorithm());
		...
		Block duplicateNonceBlock = Block.newBuilder()
				...
				.addRequestMeta(ClientRequestMeta.newBuilder().setClientId("client1").setRequestId(10)
						.build())
				.addRequestMeta(ClientRequestMeta.newBuilder().setClientId("client1").setRequestId(11)
						.build())
				.build();
		...
		assertNull(coordinator.getTree().getBlock(duplicateNonceBlock.getId()),
				"duplicate nonce block must not be cached");
