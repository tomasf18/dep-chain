...
public class UdpFairLossLink implements Link {
    ...
    public UdpFairLossLink(ProcessInfo selfInfo, ProcessConfig config, ArtificialFaultConfig faultConfig) {
        ...
        try {
            this.socket = new DatagramSocket(selfInfo.port);
        } catch (Exception e) {
    ...
    public void send(String destinationId, byte[] payload) {
        ...
        ProcessInfo destInfo = config.processes.get(destinationId);
        if (destInfo == null) {
            throw new IllegalArgumentException("Unknown destination: " + destinationId);
        } 
        try {
            DatagramPacket packet = new DatagramPacket(payload, payload.length, InetAddress.getByName(destInfo.host), destInfo.port);
            socket.send(packet);
