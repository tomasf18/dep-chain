...
import ist.depchain.network.utils.KeyLoader;
...
public class Usage {
    ...
    private static NetworkConfig loadConfiguration(String configFile, String selfId) throws IOException {
        ...
        config.faultConfig = new ArtificialFaultConfig(
                ...
                faultJson.get("packetTamperRate").getAsDouble(),
    ...
    private static void startRouter(NetworkConfig config) throws Exception {
        ProcessInfo selfInfo = config.processes.get(config.selfId);
        ProcessConfig processConfig = new ProcessConfig(config.processes);

        // Load keys
        PrivateKey privateKey = KeyLoader.loadPrivateKey(config.selfId);
        Map<String, PublicKey> trustedKeys = new HashMap<>();
        for (String peerId : config.processes.keySet()) {
            trustedKeys.put(peerId, KeyLoader.loadPublicKey(config.selfId, peerId));
        }
        ...
        Link perfectLink = new AuthenticatedPerfectLink(config.selfId, stubbornLink, fairLossLink, privateKey, trustedKeys);
